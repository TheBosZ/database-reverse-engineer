...
class MysqlSchemaParser extends BaseSchemaParser {
	...
	@override
	Future<int> parse(Database database, [Object task = null]) async {
		DDOStatement stmt = await _dbh.query('SHOW TABLES');
		DDOResult row;
		...
		await Future.wait(tables.map((Table t) => _addColumns(t)));
		await Future.wait(tables.map((Table t) => addForeignKeys(t)));
		await Future.wait(tables.map((Table t) => addIndexes(t)));
		await Future.wait(tables.map((Table t) => addPrimaryKey(t)));
		if (_addVendorInfo) {
			await Future.wait(tables.map((Table t) => addTableVendorInfo(t)));
		}

		return tables.length;
	}

	Future<DDOStatement> _getColumnDefinitions(Table table) =>
		_dbh.query("SHOW COLUMNS FROM `${table.getName()}`");

	Future<DDOStatement> _getForeignKeyDefinitions(Table table) =>
		_dbh.query("SHOW CREATE TABLE `${table.getName()}`");

	Future<DDOStatement> _getIndexDefinitions(Table table) =>
		_dbh.query("SHOW INDEX FROM `${table.getName()}`");

	Future<DDOStatement> _getPrimaryKeyDefinitions(Table table) =>
		_dbh.query("SHOW KEYS FROM `${table.getName()}`");

	Future<DDOStatement> _getVendorInfoDefinitions(Table table) =>
		_dbh.query("SHOW TABLE STATUS LIKE '${table.getName()}'");
...
WHERE TABLE_NAME='${column.getTable().getName()}'
	...
	Future _addColumns(Table table) async {
		String databaseName = table.getDatabase().getName();

		DDOStatement stmt = await _getColumnDefinitions(table);
		...
		while ((r = stmt.fetch(DDO.FETCH_ASSOC)) != null) {
			...
			if (exp.hasMatch(row['Type'])) {
				...
				if (match.groupCount > 2) {
					...
					if (cpos != -1) {
						size = precision =
							int.parse(match.group(2).substring(0, cpos));
			...
			String defaultValue = nativeType.contains(
				new RegExp(r'blob$|text$')) ? null : row['Default'];

			String propelType = getMappedPropelType(nativeType);
			if ([PropelTypes.INTEGER, PropelTypes.BIGINT].contains(
				propelType) && row['Comment'] != null &&
				row['Comment'] is String &&
				row['Comment'].indexOf('timestamp') == 0) {
				propelType = PropelTypes.INTEGER_TIMESTAMP;
			} else if (propelType == null) {
				propelType = Column.DEFAULT_TYPE;
				warn('Column [${table
					.getName()}.${name}] has a column type (${nativeType}) that the parser does not support.');
			}
			...
			if (defaultValue != null) {
				...
				column.getDomain().setDefaultValue(
					new ColumnDefaultValue(defaultValue, type));
			}
	...
	Future addForeignKeys(Table table) async {
		DDOStatement stmt = await _getForeignKeyDefinitions(table);
		...
		while ((row = stmt.fetch(DDO.FETCH_ASSOC)) != null) {
			...
			if (regEx.hasMatch(row.row.values.elementAt(1))) {
				for (Match match in regEx.allMatches(
					row.row.values.elementAt(1))) {
					...
					if (fkey != null && fkey.isNotEmpty) {
						for (String fkaction in fkactions.keys) {
							String result;
							RegExp r = new RegExp(
								"${fkaction} (${ForeignKey.CASCADE}|${ForeignKey
									.SETNULL})");
							if (r.hasMatch(fkey)) {
								fkactions[fkaction] =
									r.firstMatch(fkey).group(1);
							}
						}
					}
					...
					if (!foreignKeys.containsKey(name)) {
						...
						fk.setForeignTableCommonName(
							foreignTable.getCommonName());
					...
					for (int x = 0; x < localColumns.length; ++x) {
						foreignKeys[name].addReference(
							localColumns.elementAt(x),
							foreignColumns.elementAt(x));
					}
				}
			}
		}
	...
	Future addIndexes(Table table) async {
		DDOStatement stmt = await _getIndexDefinitions(table);
	...
	Future addPrimaryKey(Table table) async {
		DDOStatement stmt = await _getPrimaryKeyDefinitions(table);
		DDOResult row;
		while ((row = stmt.fetch(DDO.FETCH_ASSOC)) != null) {
	...
	Future addTableVendorInfo(Table table) async {
		DDOStatement stmt = await _getVendorInfoDefinitions(table);
